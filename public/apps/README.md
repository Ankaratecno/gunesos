...
## PeerJS Geçişi (WSS kaldırıldı)

Eski `chatHost` / `countHost` WebSocket sunucuları (topsportlivers) çalışmıyor;
sinyalleşme tamamen **PeerJS**'e taşındı.

### Dosyalar
- `sohbeto-peer.js` — yeni taşıma katmanı (`window.SohbetoPeer`).
- `sohbeto-engine.js` — `wsSend`, `connectChat`, `connectCountServer` artık
  PeerJS üzerinden çalışır; iç protokol (`MSG###`, `SEC###`, `[P2P_...]`) aynen korundu.
- Yükleme sırası (her tema HTML'inde):
  ```html
  <script src="https://unpkg.com/peerjs@1.5.4/dist/peerjs.min.js"></script>
  <script src="./sohbeto-peer.js"></script>
  <script src="./sohbeto-adapter.js"></script>
  <script src="./sohbeto-engine.js"></script>
  ```

### Kimlik
Peer ID deterministiktir: `sohbeto-<numaranın rakamları>`
(örn. `+90 555 111 00 01` → `sohbeto-905551110001`). Böylece numarayı bilen
karşı tarafa sunucu araması olmadan doğrudan bağlanılır.

### Test notu
Broker bağlantısı ve ID atama doğrulandı ("Ağ ID: sohbeto-…", "Sunucuya bağlandı").
Sandbox/CI ortamında UDP kapalı olduğu için WebRTC data channel açılmaz —
bu ortam kısıtıdır, kodla ilgili değildir. Kısıtlı ağlarda `CONFIG.iceServers`
içine bir TURN sunucusu eklenmelidir.
